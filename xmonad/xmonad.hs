...
-- Rofi Menus
rofi_asroot         = spawn "~/.xmonad/applets/bin/asroot"
...
myManageHook = composeAll . concat $
    ...
    my4Shifts = ["code"]
    -- my5Shifts = ["Gimp", "feh"]
    my6Shifts = ["lxappearance"]
    -- my7Shifts = ["Virtualbox"]
    -- my8Shifts = ["Thunar"]
    --my9Shifts = [""]
    my10Shifts = ["telegram-desktop", "TelegramDesktop"]




myLayout = spacingRaw False (Border 20 0 20 0) True (Border 0 20 0 20) True $ avoidStruts $ mkToggle (NBFULL ?? NOBORDERS ?? EOT) $ tiled ||| Mirror tiled ||| spiral (6/7)  ||| ThreeColMid 1 (3/100) (1/2) ||| Full where
...
--  , ((0, xF86XK_AudioStop), spawn $ "mpc stop")
  ...
  , ((0, xF86XK_AudioStop), spawn $ "playerctl stop")
    ...
    , ((modMask,                    xK_n),      rofi_network_menu)
    , ((modMask,                    xK_x),      rofi_powermenu)
    , ((mod1Mask .|. controlMask,   xK_m),      rofi_mpd)
    , ((modMask,                   xK_s),      rofi_screenshot)
    , ((mod1Mask .|. controlMask,   xK_r),      rofi_asroot)
    , ((modMask,                    xK_z),      rofi_windows)
  ...
  , ((controlMask .|. shiftMask , xK_h), sendMessage (IncMasterN (1)))

  -- Expand the master area.
  , ((controlMask .|. shiftMask , xK_l), sendMessage (IncMasterN (-1)))
